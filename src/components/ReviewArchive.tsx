...
export function ReviewArchive({ selectedDate, onSelectDate }: { selectedDate: string; onSelectDate: (date: string) => void }) {
  ...
  return <div className="review-archive">
    <section className="card review-history"><div className="card-head"><div><p className="eyebrow">历史复盘</p><h2>完整记录与周期模板</h2></div><span>{history.length} 篇</span></div>
      ...
      <div className="period-review-actions"><span>{notice}</span><div className="data-actions"><button className="btn quiet" aria-label={`导出${kind === 'week' ? '周' : '月'}复盘`} onClick={() => void exportPeriod()}><Download /> 导出{kind === 'week' ? '周' : '月'}复盘模板</button><button className="btn quiet" onClick={() => setExportOpen(true)}><Download /> 按日期导出</button></div></div>
