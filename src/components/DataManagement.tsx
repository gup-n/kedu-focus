...
function ActionButton({ children, onClick, primary = false, disabled = false }: { children: React.ReactNode; onClick: () => void; primary?: boolean; disabled?: boolean }) {
  return <button type="button" className={`btn ${primary ? 'primary' : 'quiet'}`} onClick={onClick} disabled={disabled}>{children}</button>
}
...
export function DataManagement() {
  ...
  function applyState(next: AppState, message: string) {
    downloadBackup(rawState, '刻度_导入前备份')
  ...
  function overwrite() {
    ...
    applyState(envelope.data, '已覆盖本地数据，并已自动下载导入前备份。')
  }
  ...
  function prepareMerge() {
    ...
    if (!nextPlan.conflicts.length) {
      applyState(nextPlan.state, '数据已合并，并已自动下载导入前备份。')
      return
    }
    setPlan(nextPlan)
    setChoices({})
  ...
  function applyMerge() {
    if (!plan) return
    try {
      applyState(applyConflictChoices(plan, choices), '冲突已按你的选择合并，并已自动下载导入前备份。')
    } catch (reason) {
  ...
  return <>
    ...
    <div className="data-actions">
      <ActionButton onClick={() => downloadBackup(rawState)}><Download/> 导出 JSON 备份</ActionButton>
    ...
    {error && !envelope && <p className="form-error" role="alert">{error}</p>}
    ...
    {showCsv && <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && setShowCsv(false)}><div className="dialog data-dialog">
      ...
      <div className="csv-options">{([['tasks', '任务'], ['sessions', '专注记录'], ['sleep', '睡眠记录']] as [CsvKind, string][]).map(([kind, label]) => <ActionButton key={kind} onClick={() => downloadCsv(rawState, kind)}><Download/> 导出{label}</ActionButton>)}</div>
    </div></div>}
    ...
    {envelope && preview && <div className="modal-backdrop"><div className="dialog import-dialog">
      ...
      {!plan ? <>
        ...
        <div className="dialog-actions"><ActionButton onClick={overwrite}>覆盖本地</ActionButton><ActionButton primary onClick={prepareMerge}>合并数据</ActionButton></div>
      </> : <>
        ...
        <div className="dialog-actions"><ActionButton onClick={() => { setPlan(null); setChoices({}); setError('') }}>返回预览</ActionButton><ActionButton primary disabled={resolved !== plan.conflicts.length} onClick={applyMerge}>应用合并</ActionButton></div>
      </>}
    </div></div>}
  </>
}
